...
-- Dumping structure for view ems_kmscl.v_bidder_details
...
-- Dumping structure for view ems_kmscl.v_bidder_details_2
...
-- Dumping structure for view ems_kmscl.v_bidder_equiplist
...
-- Dumping structure for view ems_kmscl.v_bidder_price
...
-- Dumping structure for view ems_kmscl.v_bidder_selection_status
...
-- Dumping structure for view ems_kmscl.v_bidder_spare_price
...
-- Dumping structure for view ems_kmscl.v_cur_year
...
-- Dumping structure for view ems_kmscl.v_fundreceived_schemewise
...
-- Dumping structure for view ems_kmscl.v_govtagency
...
-- Dumping structure for view ems_kmscl.v_govtpurchase
...
-- Dumping structure for view ems_kmscl.v_invoice
...
-- Dumping structure for view ems_kmscl.v_invoice_process
...
CREATE TABLE `v_invoice_process` (
	...
	`new_ittds` DECIMAL(20,2) NOT NULL,
...
-- Dumping structure for view ems_kmscl.v_inv_prs
...
-- Dumping structure for view ems_kmscl.v_itequip_details
...
CREATE TABLE `v_itequip_details` (
	...
	`ite_stockregno` VARCHAR(500) NULL COLLATE 'latin1_swedish_ci',
...
-- Dumping structure for view ems_kmscl.v_loi
...
-- Dumping structure for view ems_kmscl.v_loisch_details
...
-- Dumping structure for view ems_kmscl.v_loisch_itemwise
...
-- Dumping structure for view ems_kmscl.v_mainequip_price
...
-- Dumping structure for view ems_kmscl.v_master_equipment
...
-- Dumping structure for view ems_kmscl.v_master_institution
...
-- Dumping structure for view ems_kmscl.v_master_project
...
-- Dumping structure for view ems_kmscl.v_procure_details
...
-- Dumping structure for view ems_kmscl.v_procure_item_details
...
-- Dumping structure for view ems_kmscl.v_purchase
...
-- Dumping structure for view ems_kmscl.v_purchase_order
...
-- Dumping structure for view ems_kmscl.v_purchase_qty
...
-- Dumping structure for view ems_kmscl.v_purchase_schdule
...
-- Dumping structure for view ems_kmscl.v_sanction_allotment
...
-- Dumping structure for view ems_kmscl.v_sanction_details
...
-- Dumping structure for view ems_kmscl.v_sanction_equip
...
-- Dumping structure for view ems_kmscl.v_sanction_order
...
-- Dumping structure for view ems_kmscl.v_sanction_order_projects
...
-- Dumping structure for view ems_kmscl.v_sanction_order_schemes
...
-- Dumping structure for view ems_kmscl.v_sd_details
...
-- Dumping structure for view ems_kmscl.v_spare_price
...
-- Dumping structure for view ems_kmscl.v_tag_details
...
-- Dumping structure for view ems_kmscl.v_workint
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bidder_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bidder_details_2` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bidder_equiplist` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bidder_price` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bidder_selection_status` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_bidder_spare_price` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_cur_year` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_fundreceived_schemewise` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_govtagency` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_govtpurchase` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_invoice` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_invoice_process` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_inv_prs` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_itequip_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_loi` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_loisch_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_loisch_itemwise` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_mainequip_price` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_master_equipment` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_master_institution` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_master_project` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_procure_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_procure_item_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_purchase` AS -- Non Govt Agency Order Details
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_purchase_order` AS -- LOI REQ FOR TENDER
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_purchase_qty` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_purchase_schdule` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sanction_allotment` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sanction_details` AS SELECT
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sanction_equip` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sanction_order` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sanction_order_projects` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sanction_order_schemes` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_sd_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_spare_price` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_tag_details` AS SELECT 
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `v_workint` AS SELECT 
